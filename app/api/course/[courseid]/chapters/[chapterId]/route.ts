...
export async function PATCH(req:Request,{params}:params_props)
{
    ...
    if(!courseOwner)return new NextResponse('Unauthorized',{status:403})
      ...
      if(values.videoUrl)
      {
                ...
                if(existingMuxData)
                {
                    console.log('existing muxdata')
                  /*  const delete_data= await Video.Assets.del(existingMuxData.assetId);
                   console.log('delete_data',delete_data) */
                   const delete_db_mux_data= await db.muxData.delete({
                        where:{id:existingMuxData.id}
                    })
                    console.log('delete_db_mux_data',delete_db_mux_data)
                }
